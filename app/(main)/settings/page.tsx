...
import { auth, SignOutButton, useAuth, useClerk } from "@clerk/nextjs";
...
function Page() {
  ...
  useEffect(() => {
    const checkRole = async () => {
      if (userId) {
        const isTeacherResult = await isTeacher(userId);
        setIsTeacherAccount(isTeacherResult);

        const isAdminResult = await isAdmin(userId)
        setIsAdminAccount(isAdminResult)
      }
    };
    ...
    <div className="w-full p-10 lg:p-22  bg-secondary min-h-screen rounded-t-3xl md:rounded-tl-3xl">
      <div className="container max-w-[770px]">
        ...
        {isAdminAccount  && (
          <div className="flex flex-row items-center justify-between rounded-lg border p-4">
            <div className="space-y-0.5">
              ...
              <p>You are an admin you can go to admin dashboard</p>
            </div>
            <div>
              <Button asChild>
        ...
        >
          ...
          <div>
            <Button variant="destructive" className="mx-auto" onClick={handleLogout}>
              Sign out
            </Button>
          </div>
        </Alert>
